...
*FASPA* contains several R-scripts that allow transformation of the output files from **FASP_uparse.sh** and **FASP_unoise.sh** into a single *phyloseq-object*. From there on, the script **Physeq_to_Rhea.R** further transforms the *phyloseq-object* into a *OTU table with taxonomy* which is already formatted according to the needs of the Rhea pipeline.  
...
Rhea can be downloaded from this link (https://lagkouvardos.github.io/Rhea/). With the downloaded files you will also find a .pdf file with detailed instructions on how to apply each of the Rhea scripts. Choose the file location (In windows or linux) where you want to perform your analysis. e.g. C:\RheaTest\.
Download Sintaxthe file Physeq_to_Rhea.R from this respository into the folder.
In this folder you have to prepare six empty folders, named exactly as shown in the picture below.

Additionally, we need to install the package tidyr (select from your closest cran repository). 
**Files needed**
...
John Quensen (2018). RDPutils: R Utilities for Processing RDPTool
  Output. R package version 1.4.1.
